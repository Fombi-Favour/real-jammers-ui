...
const RegisterInput = () => {
  ...
  const UploadImage = (e) => {
    ...
    const storageRef = ref(storage, `Image/${Date.now()}-${pic.name}`);
        ...
        <form className="p-6" onSubmit={handleSubmit}>
          ...
          <div className="p-1 flex rounded bg-gray-800 mt-2">
            <input
              ...
              className="p-1 px-2 appearance-none text-gray-200 bg-gray-800 outline-none w-full"
              required
            />
          </div>
          {/* Last Name field */}
          ...
          <div className="p-1 flex rounded bg-gray-800 mt-2">
            ...
            />
            <FaEye
              className="text-slate-100 m-2"
              size={20}
              onClick={() => setShowPassword((prevState) => !prevState)}
            />
          </div>
          {/* Button Sign up */}
          <button className="m-3 p-3 w-60 bg-teal-500 shadow-lg shadow-teal-500/50 hover:shadow-teal-500/40">
            Sign Up
          </button>
        </form>
      </div>
